...
namespace ProtoBuf.Meta
{
    ...
    public abstract class TypeModel
    {
        ...
        /// <param name="value">The existing instance to be serialized (cannot be null).</param>
        /// <param name="dest">The destination writer to write to.</param>
        public void CustomSerialize(ProtoWriter dest, object value)
        {
#if FEAT_IKVM
            throw new NotSupportedException();
#else
            if (dest == null) throw new ArgumentNullException("dest");
            if (!dest.NetCache.IsRootObjectAssigned)
            {
                dest.SetRootObject(value);
            }
            SerializeCore(dest, value);
        ...
        public ProtoReader CreateReader(Stream source, PrefixStyle style, SerializationContext context)
        {
            return ProtoReader.Create(source, this, context, 0);
        }

        public void ReclaimReader(ProtoReader reader)
        {
            ProtoReader.Recycle(reader);
        }

        public int PrepareRead(Stream source, Type type, PrefixStyle style, int expectedField, Serializer.TypeResolver resolver)
        {
            bool skip;
            int len;
            int tmpBytesRead;
            ...
            do
            {
                ...
                bool expectPrefix = expectedField > 0 || resolver != null;
                len = ProtoReader.ReadLengthPrefix(source, expectPrefix, style, out actualField, out tmpBytesRead);
                if (tmpBytesRead == 0 || len<0) return -1;
        ...
        public object DoDeserialize(ProtoReader reader,int dataLenth, object value, Type type)
        {
            reader.LightInit(dataLenth);
            int key = GetKey(ref type);
            if (key >= 0 && !Helpers.IsEnum(type))
            {
                value = Deserialize(key, value, reader);
            ...
            {
                if (!(TryDeserializeAuxiliaryType(reader, DataFormat.Default, Serializer.ListItemTag, type, ref value, true, false, true, false) || dataLenth == 0))
                {
                    TypeModel.ThrowUnexpectedType(type); // throws
                }
            }
