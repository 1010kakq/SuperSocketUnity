...
namespace SuperSocket.Client.Proxy
{
    ...
    public abstract class ProxyConnectorBase : ConnectorBase
    {
        ...
        protected abstract UniTask<ConnectState> ConnectProxyAsync(EndPoint remoteEndPoint, ConnectState state, CancellationToken cancellationToken);
        ...
        /// <param name="state">The connection state.</param>
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A UniTask representing the asynchronous connection operation. The result contains information about the connection status.</returns>
        /// <remarks>
