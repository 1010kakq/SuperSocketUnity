...
namespace SuperSocket.Connection
{
    ...
    public abstract partial class PipeConnectionBase : ConnectionBase, IConnection, IPipeConnection
    {
        ...
        public async override IAsyncEnumerable<TPackageInfo> RunAsync<TPackageInfo>(IPipelineFilter<TPackageInfo> pipelineFilter)
        {
            ...
            var readTaskCompletionSource = new TaskCompletionSource<object>();

            _cts.Token.Register(() =>
            {
                readTaskCompletionSource.TrySetResult(null);
            });
            ...
            while (true)
            {
                ...
                try
                {
                    read = await packagePipeEnumerator.MoveNextAsync();
                }
        ...
        private void FireClose()
        {
            if (!_isDetaching && !IsClosed)
            {
                ...
                catch (Exception exc)
                {
                    if (!IsIgnorableException(exc))
                    {
                        ...
                        if (!CloseReason.HasValue)
                        {
                            ...
                            {
                                CloseReason = Connection.CloseReason.SocketError; 
                            }
                            else
                            {
                                CloseReason = Connection.CloseReason.ApplicationError; 
                            }
                        }
                    }
                }
                ...
                {
                    OnClosed();
                    //UnityEngine.Debug.LogError($"FireClose: {_isDetaching} {IsClosed}");
                }
            }
        }
        ...
        public override async UniTask CloseAsync(CloseReason closeReason)
        {
        ...
        protected async Task CancelAsync()
        {
            ...
            _cts.Cancel();
            await CompleteWriterAsync(OutputWriter, _isDetaching);
        }
        ...
        public override async UniTask SendAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
        {
        ...
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A UniTask that represents the asynchronous send operation.</returns>
        public override async UniTask SendAsync(ReadOnlySequence<byte> buffer, CancellationToken cancellationToken = default)
        {
            var sendLockAcquired = false;
        ...
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A UniTask that represents the asynchronous send operation.</returns>
        public override async UniTask SendAsync<TPackage>(IPackageEncoder<TPackage> packageEncoder, TPackage package, CancellationToken cancellationToken = default)
        {
            var sendLockAcquired = false;
        ...
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A UniTask that represents the asynchronous send operation.</returns>
        public override async UniTask SendAsync(Action<PipeWriter> write, CancellationToken cancellationToken)
        {
            CheckConnectionSendAllowed();
        ...
        protected async IAsyncEnumerable<TPackageInfo> ReadPipeAsync<TPackageInfo>(PipeReader reader, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            await CompleteReaderAsync(reader, _isDetaching);
            yield break;
        }
        ...
        /// </summary>
        /// <returns>A UniTask that represents the asynchronous detach operation.</returns>
        public override async UniTask DetachAsync()
        {
            _isDetaching = true;
            await CancelAsync();
        ...
        protected virtual async UniTask CompleteReaderAsync(PipeReader reader, bool isDetaching)
        {
        ...
        /// <param name="isDetaching">Indicates if this operation is a part of detaching action.</param>
        protected virtual async UniTask CompleteWriterAsync(PipeWriter writer, bool isDetaching)
        {
