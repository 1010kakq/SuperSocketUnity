...
namespace SuperSocket.Connection
{
    ...
    public abstract class PipeConnection : PipeConnectionBase
    {
        ...
        {
            await UniTask.WhenAll(FillInputPipeAsync(Input.Writer, cancellationToken).AsUniTask(), ProcessSends().AsUniTask());
        ...
        internal virtual async Task FillInputPipeAsync(PipeWriter writer, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var bytesRead = await FillInputPipeWithDataAsync(memory, cancellationToken);
                    ...
                    if (bytesRead > 0)
                    {
                        try
                        {
                            var dataSlice = memory.Slice(0, bytesRead);
                            if (MemoryMarshal.TryGetArray<byte>(dataSlice, out var segment))
                            {
                                string pipeHex = NetLogUtil.HexDump(segment.Array, segment.Offset, Math.Min(bytesRead, 64));
                                NetLogUtil.LogReceive($"[PipeConnection.FillInputPipeAsync] 准备写入管道的数据: bytesRead={bytesRead}\n{pipeHex}");
                                
                                // 检查MsgId
                                if (bytesRead >= 6)
                                {
                                    ushort msgId = System.BitConverter.ToUInt16(segment.Array, segment.Offset + 4);
                                    NetLogUtil.LogReceive($"[PipeConnection.FillInputPipeAsync] 管道写入MsgId: {msgId}");
                                    
                                    if (msgId == 499)
                                    {
                                        NetLogUtil.LogReceive($"[PipeConnection.FillInputPipeAsync] *** 管道写入MsgId=499数据 ***");
                                    }
                                }
                            }
                        }
                        ...
                        {
                            NetLogUtil.LogError($"[PipeConnection.FillInputPipeAsync] 管道写入日志错误: {e.Message}");
                        }
                    }
#endif
        ...
        protected async ValueTask<bool> ProcessOutputRead(PipeReader reader)
        {
            ...
            if (!buffer.IsEmpty)
            {
                try
                {
                    ...
                    var timeoutTask = UniTask.Delay(sendTimeout);
                    var (hasResultLeft, winnerResult) = await UniTask.WhenAny(sendTask, timeoutTask);
                    
                    if (!hasResultLeft) // timeout task completed first
                    {
                        throw new TimeoutException($"Send operation timed out after {sendTimeout.TotalMilliseconds}ms");
                    }
                    
                    // Use the result from WhenAny instead of awaiting sendTask again
                    var bytesSent = winnerResult;
        ...
        protected abstract UniTask<int> SendOverIOAsync(ReadOnlySequence<byte> buffer, CancellationToken cancellationToken);
        ...
        protected abstract UniTask<int> FillInputPipeWithDataAsync(Memory<byte> memory, CancellationToken cancellationToken);
    }
}
