...
namespace SuperSocket
{
    ...
    public static class AsyncParallel
    {
        ...
        public static async UniTask ForEach<TItem>(IEnumerable<TItem> source, Func<TItem, UniTask> operation, int maxDegreeOfParallelism = 5)
        {
        ...
        /// <summary>
        /// [OBSOLETE] Executes the specified asynchronous operation for each item in the source collection in parallel.
        /// Use the UniTask overload for better Unity integration.
        /// </summary>
        /// <typeparam name="TItem">The type of the items in the source collection.</typeparam>
        /// <param name="source">The source collection of items.</param>
        /// <param name="operation">The asynchronous operation to execute for each item.</param>
        ...
        [System.Obsolete("Use the UniTask overload with Func<TItem, UniTask> for better Unity integration.")]
        public static async Task ForEach<TItem>(IEnumerable<TItem> source, Func<TItem, Task> operation, int maxDegreeOfParallelism = 5)
        {
            await ForEachTask(source, operation, new ParallelOptions
            {
                MaxDegreeOfParallelism = maxDegreeOfParallelism,
                CancellationToken = CancellationToken.None
            });
        ...
        /// <returns>A UniTask that represents the asynchronous operation.</returns>
        public static async UniTask ForEach<TItem>(IEnumerable<TItem> source, Func<TItem, UniTask> operation, ParallelOptions parallelOptions)
        {
            ...
            foreach (var item in source)
            {
                ...
                allTasks.Add(UniTask.Run(async () =>
                {
                    try
                    {
        ...
        /// <summary>
        /// [OBSOLETE] Executes the specified asynchronous operation for each item in the source collection in parallel with the specified parallel options.
        /// Use the UniTask overload for better Unity integration.
        /// </summary>
        /// <typeparam name="TItem">The type of the items in the source collection.</typeparam>
        /// <param name="source">The source collection of items.</param>
        ...
        {
            await ForEachTask(source, operation, parallelOptions);
        }

        /// <summary>
        /// Internal implementation for Task-based operations.
        /// </summary>
        private static async Task ForEachTask<TItem>(IEnumerable<TItem> source, Func<TItem, Task> operation, ParallelOptions parallelOptions)
        {
            var allTasks = new List<UniTask>();
            var throttler = new SemaphoreSlim(initialCount: parallelOptions.MaxDegreeOfParallelism);
